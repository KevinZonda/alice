...
- `internal/bootstrap`
  负责构建每个 bot 的运行时对象，并把 prompt loader、runtime API 鉴权、config reload 等横切能力接起来。
...
- `internal/bootstrap`
  runtime 构建、路径解析、认证检查、skill 释放、config reload
- `internal/config`
...
- request body 大小限制
- 进程内 auth rate limit
- 本地上传仍要求文件可读、非空且不是目录，并继续受飞书大小限制约束
...
- `run_llm`
- `run_llm` 支持 `state_key`（Codex/Kimi 稳定 thread ID）、`resume_session_key`（指定回复 channel）、sticky thread（自动更新 `resume_thread_id`）
